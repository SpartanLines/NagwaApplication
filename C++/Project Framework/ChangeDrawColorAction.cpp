...
void ChangeDrawColorAction::Execute()
{
	...
	while (j != pManager->getfigcount())
	{
		...
		if (selected != NULL)
		{
			...
			selected->ChngDrawClr(newcolor);
			arr[k].id = selected->GetID();
			arr[k].h = selected->GetDrawClr();
			k++;
		}
	}
	if (selected == NULL)
	{
		...
		else UI.DrawColor = newcolor;
	}

}
void ChangeDrawColorAction::Undo()
{
	...
	{
		UI.DrawColor = oldfill;
	}

}

void ChangeDrawColorAction::Redo()
{
